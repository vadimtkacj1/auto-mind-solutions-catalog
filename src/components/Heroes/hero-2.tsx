  ...
  useEffect(() => {
    ...
    const fragmentShaderSource = `
      ...
      vec3 getBackground(vec3 rd) {
        ...
        nebula += vec3(0.2, 0.5, 0.3) * pow(max(0.0, sin(rd.x * 2.0 + uTime * 0.1)), 3.0) * 0.2;
        nebula += vec3(0.15, 0.4, 0.3) * pow(max(0.0, sin(rd.y * 2.5 + uTime * 0.05)), 3.0) * 0.2;
        ...
        if(t < MAX_DIST) {
          ...
          if(length(refractDir) > 0.0) {
            ...
            if(t2 < MAX_DIST) {
              ...
              vec3 bgR = getBackground(r) * vec3(1.0, 1.5, 1.2);
              vec3 bgG = getBackground(g) * vec3(1.2, 1.5, 1.0);
              vec3 bgB = getBackground(b) * vec3(1.0, 1.2, 1.4);
    ...
    <section className="relative overflow-hidden h-screen bg-black">
      ...
      <div className="relative z-10 flex items-center justify-center h-full">
        <div className="text-center pointer-events-none px-4">
          <h1 className="text-[clamp(4.5rem,13vw,11rem)] font-black mb-2 tracking-tighter leading-none bg-gradient-to-br from-white via-gray-100 to-white bg-clip-text text-transparent animate-pulse-glow"
              style={{
                filter: 'drop-shadow(0 0 40px rgba(255, 255, 255, 0.4)) drop-shadow(0 0 80px rgba(16, 185, 129, 0.3))',
                animation: 'glowPulse 3s ease-in-out infinite alternate'
              }}>
            אמרלד
          </h1>
          <p className="text-[clamp(0.9rem,2vw,1.2rem)] font-light text-white/90 tracking-[0.3em] uppercase mb-10"
             style={{ textShadow: '0 0 30px rgba(255, 255, 255, 0.5), 0 0 60px rgba(16, 185, 129, 0.3)' }}>
            ספקטרום של אפשרויות
          </p>

          <div className="flex flex-col sm:flex-row gap-6 justify-center pointer-events-auto">
            <GlassButton>גלה עוד</GlassButton>
            <GlassButton>הצטרף עכשיו</GlassButton>
          </div>
        </div>
      </div>
      ...
      <style>{`
        @keyframes glowPulse {
          ...
          to {
            filter: drop-shadow(0 0 60px rgba(255, 255, 255, 0.6)) drop-shadow(0 0 120px rgba(5, 150, 105, 0.4));
          }
        }
      `}</style>
    </section>
  ...
  return (
    ...
    >
      ...
      <span
        ...
        style={{
          padding: '1.5px',
          background: 'linear-gradient(135deg, rgba(255, 255, 255, 0.4) 0%, rgba(16, 185, 129, 0.4) 25%, rgba(5, 150, 105, 0.4) 50%, rgba(16, 185, 129, 0.4) 75%, rgba(255, 255, 255, 0.4) 100%)',
